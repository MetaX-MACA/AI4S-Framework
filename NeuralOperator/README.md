...
FNO（Fourier Neural Operator，傅里叶神经算子）是该库中最具代表性的神经算子模型。FNO 的核心思想是将神经网络的运算迁移到傅里叶频域完成：通过傅里叶变换将输入函数映射到频域，在傅里叶空间执行线性变换完成全局信息交互，再经逆傅里叶变换回到物理空间，搭配非线性激活层构建完整的算子层。FNO 天然具备对网格分辨率不敏感的关键特性，训练完成后可以在不同于训练集的离散网格上直接推理预测 PDE 解，突破传统 CNN 受限于固定网格尺寸的约束，广泛用于流体力学、传热、弹性力学等各类偏微分方程正向求解、场预测任务，也是 AI for Science 领域求解 PDE 的主流基线模型。
